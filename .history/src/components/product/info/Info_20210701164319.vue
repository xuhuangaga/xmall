    ...
    <div class="flex-j-around a-center content bg-c-w">
      ...
      <div class="content_r">
        ...
        <div class="num f-size14 flex a-center">
          ...
          <div>
            <input type="text" class="num_ipt" v-model="num" @blur="blur" />
          </div>
          <div
        ...
        <div class="flex-a-center btn_dv f-size12 m-t20">
          <div class="addCar f-c-w t-a-center c-hand" :class="{'buy_btn':tipShow}">加入购物车</div>
          <div class="buy t-a-center c-hand m-l10" :class="{'buy_btn':tipShow}">立即购买</div>
        </div>
      </div>
    </div>
  </div>
  ...
  watch: {
    num(val1, val2) {
      ...
      if (Number(val1) > this.list.limitNum) {
        // this.$message({
        //   type: "error",
        //   message: `购买数量不能超过${this.list.limitNum}`,
        //   duration: 1000
        // });
        // setTimeout(item => {
        //   this.num = val2;
